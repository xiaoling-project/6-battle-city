...
class MyPanel extends JPanel implements KeyListener, Runnable{
	...
	public MyPanel() {
		...
		for (int i = 0, index = 0; i < Record.getHostileTankNumber(); i++) {
			...
			if (i < 8) {
				newHt = new HostileTank(i * 50, 0, HostileTank.DOWM, nowTankColor, 3,
						Tank.TankColorToLiveTable(nowTankColor));
			} else {
				int temp = i % 6;
				newHt = new HostileTank(i * 50, temp * 50, HostileTank.LEFT, nowTankColor, 5,
						Tank.TankColorToLiveTable(nowTankColor));
			}
			newHt.setHts(hts);
			newHt.setMt(mt);
